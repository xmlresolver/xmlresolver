 ...
 *
 * On the one hand, the {@link Resolver} class in this API is a concrete class, not suitable
 ...
 * Further complicating matters, there are conflicting interfaces for doing resolution.
 * In particular, the {@link javax.xml.stream.XMLResolver} and
 * {@link org.xml.sax.ext.EntityResolver2} interfaces
 * can't be implemented by the same class.
 *
 * This interface is an attempt to abstract away from the existing interfaces and expose
 * something that could be implemented independently that provides enough functionality t
 * implement all of the common resolver interfaces.
 */
...
public interface ResourceResolver {
    /** Resolve a URI.
     *
     * Attempts to resolve the specified href URI. If resolution fails, the href
     ...
     * is attempted with the resulting absolute URI.
     *
     ...
     *
     * Attempts to resolve the specified href URI, optionally employing the baseURI as
     ...
     *
     * If the systemId is null and the baseURI is not, the systemId is taken to be the baseURI
     * and the baseURI is treated as if it were null.
     *
     * If name, publicId and systemId are all null, null is returned.
     *
     * If systemId or publicId are not null, the method attempts to resolve an external identifier with
     ...
     * not a systemId, even in XML systems.)
     *
     * If name is not null, but systemId is, name is assumed to be document type name and
     * the method attempts to resolve an external identifier that matches. A <code>doctype</code> catalog
     * entry, for example.
     *
     * If the systemId is relative, resolution fails, and baseURI is not null, the systemId
     * is made absolute with respect to the baseURI and resolution is attempted a second time.
     *
